...
static void set_mode(uint8_t mode) {
	...
	write_register(REG_OP_MODE, FSK_OOK_MODE | MODULATION_FSK/*MODULATION_OOK*/ | mode);
	ESP_LOGD(TAG, "set_mode %d -> %d", cur_mode, mode);
	ESP_LOGD(TAG, "REG_OP_MODE = %d", read_register(REG_OP_MODE));
...
void rfm95_init(void) {
	...
	//write_register(REG_DIO_MAPPING_1, 0);
	...
	write_register(REG_BITRATE_MSB, 0xFF/*0x07*/);
	write_register(REG_BITRATE_LSB, 0xFF/*0xA1*/);
	
	write_register(0x24, 0x0);
	...
	write_register(REG_PREAMBLE_MSB, 0x0);
	write_register(REG_PREAMBLE_LSB, 0x0);
	...
	ESP_LOGD(TAG, "init!");
	write_register(REG_POWER, 15);
	write_register(REG_FDEV_LSB, 10);
	write_register(0x5d, 15);
	ESP_LOGD(TAG, "REG_FDEV_LSB = %d", read_register(REG_FDEV_LSB));// было 82 (5кHz) 
	ESP_LOGD(TAG, "REG_FDEV_MSB = %d", read_register(REG_FDEV_MSB));// было 0 
	ESP_LOGD(TAG, "REG_POWER = %d", read_register(REG_POWER));// было 79 
	ESP_LOGD(TAG, "REG_OP_MODE = %d", read_register(REG_OP_MODE));
...
{
	for (int i=0;i<94;i++)
		ESP_LOGD(TAG, "REG0x%x = %d",i, read_register(i));
}
...
void transmit(uint8_t *buf, int count) {
	...
	for (;;) {
		...
		ESP_LOGD(TAG, "REG_OP_MODE in transmit= %d", read_register(REG_OP_MODE));
		xmit(buf, avail);
